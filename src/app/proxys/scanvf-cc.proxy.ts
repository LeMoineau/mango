...
export class ScanvfCC implements Proxy {

  NB_NEW_CHAPTERS_BEFORE_RECENT_CHAPTERS = 3;

  inProductionURLstart = "https://scan-vf.cc";
  inDevURLstart = "/scanvfCC";
  currentURLstart = "";
  ...
  urlMangaInfo(mangaParsedTitle: string) {
    return `${this.currentURLstart}/manga/${mangaParsedTitle}`;
  };

  urlChapterPages(mangaParsedTitle: string, chapter: DataObject) {
    return `${this.currentURLstart}/manga/${mangaParsedTitle}/${chapter.num}`;
  };
  ...
  scrapeNewChapters(html: string, callback) {
    ...
    $(".manga-item").each((index, value) => {
      ...
      $2(".manga-chapter").each((index2, value2) => {
        ...
        chapter.image = `https://www.scan-vf.cc/uploads/manga/${chapter.parsedTitle}/cover/cover_250x350.jpg`;
  ...
  scrapeChapterPagesURL(parsedTitle: string, chapter: DataObject, html: string, callback) {
    ...
    $("#all .img-responsive").each((index, value) => {
      ...
      url = `${this.currentURLstart}/${url.split("scan-vf.cc/")[1]}`;
      urls.push(url);
    })
    callback(urls);
